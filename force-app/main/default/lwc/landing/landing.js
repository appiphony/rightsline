...
export default class Landing extends LightningElement {
    ...
    connectedCallback() {
        getPackageVersion()
            .then((responseDataString) => {
                ...
                this.packageVersion =
                    responseData.major + '.' + responseData.minor;
            })
            .catch((error) => {
    ...
    @api
    init(wizards) {
        getData().then((responseDataString) => {
            const responseData = JSON.parse(responseDataString);
            const completedSteps = JSON.parse(
                responseData.results.setupData.Steps_Completed__c
            );
    ...
    show(wizards) {
        ...
        window.setTimeout(() => {
            this.template
                .querySelectorAll('.slds-progress-ring_complete')
                .forEach((element) => {
