...
export default class Wizard extends LightningElement {
    ...
    // Setup related properties (explicit properties definitions are required for Lightning Web Security support, which doesn't like expandos)
    @api setup = false;
    @api index = 0;
    ...
    get modal() {
        const _modal = Array.from(this.querySelectorAll('c-modal')).filter(el => {
            return el.assignedSlot && el.assignedSlot.name === 'modal';
        })

        return _modal.length ? _modal[0] : this.template.querySelector('c-modal');
    }
    ...
    constructor() {
        ...
        this.template.addEventListener('primary', this.handlePrimary.bind(this));
    ...
    renderedCallback() {
        if (this._init !== true) {
            this._init = true;
            const steps = Array.from(this.querySelectorAll('*')).filter((el) => {
    ...
    next(event) {
        ...
        if (index > -1) {
            let completedSteps = JSON.parse(JSON.stringify(this.completedSteps));
    ...
    navigateTo(event) {
        ...
        this.steps.forEach(step => {
            if (!step.classList.contains('slds-hide')) {
                //TODO: need to change this to strict equality somehow
                if (step == nextStep) {
                    return;
                }
                currentStep = step;
    ...
    handlePrimary(event) {
        ...
        if (this._modalStepIndex !== null && this._modalStepIndex >= 0 && this._modalStepIndex < this.steps.length) {
